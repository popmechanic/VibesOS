...
1. **Setup**: Read eval/config.md, eval/napkin.md, eval/scoreboard.md. Create gen-N dir.
2. **Mutation**: Write directives, dispatch N mutator subagents (autoresearch-mutator) with isolation: worktree. Include variant-0 as control.
3. **Generation**: Dispatch generator subagents (autoresearch-generator) with run_in_background: true. ~4-5 concurrent.
